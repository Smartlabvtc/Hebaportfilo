    ...
    <div className={`site-shell ${isEnglish ? "is-english" : ""}`} dir={isEnglish ? "ltr" : "rtl"}>
      ...
      <main id="main-content">
        ...
        <section className="section skills-section" id="skills">
          ...
          <div className="container tools-row"><div className="tools-main"><span className="tools-label">{tx("أدوات يومية", "Everyday tools")}</span><div className="tool-list" aria-label={tx("البرامج المستخدمة يومياً", "Everyday software")}>{[{ name: "Windows", icon: "https://cdn.simpleicons.org/windows/0078D4" }, { name: "Microsoft Office", icon: "https://cdn.simpleicons.org/microsoftoffice/D83B01" }, { name: "Google Apps", icon: "https://cdn.simpleicons.org/google/4285F4" }, { name: "Canva", icon: "https://cdn.simpleicons.org/canva/00C4CC" }, { name: "Bambu Studio", icon: "https://cdn.simpleicons.org/bambulab/00AE42" }, { name: "Beam Studio", mark: "B", tone: "violet" }].map(tool => <span className="tool-chip" key={tool.name}>{tool.icon ? <img src={tool.icon} alt="" aria-hidden="true" loading="lazy" /> : <b className={`tool-mark ${tool.tone}`}>{tool.mark}</b>}<span>{tool.name}</span></span>)}</div></div><div className="tools-badge"><Atom size={18} /> <span>{tx("تعلم مستمر", "Continuous learning")}</span></div></div>
